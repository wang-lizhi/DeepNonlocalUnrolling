                ...
                for k, v in state.items():
                    if isinstance(v, torch.Tensor):
                        if device_id == -1:
                            ...
                            state[k] = v.cuda(device=device_id)
            continue
...
def load_ckpt(modules_optims, ckpt_file, load_to_cpu=True, verbose=True):
    ...
    return ckpt['ep'], ckpt['scores']#, ckpt['lut']
...
def load_state_dict(model, src_state_dict):
    ...
    for name, param in src_state_dict.get('state_dicts')[0].items():
    ...
    src_missing = set(dest_state_dict.keys()) - set(src_state_dict.get('state_dicts')[0].keys())
    if len(src_missing) > 0:
        print("Keys not found in source state_dict: ")
        for n in src_missing:
            print('\t', n)

    dest_missing = set(src_state_dict.get('state_dicts')[0].keys()) - set(dest_state_dict.keys())
...
def adjust_lr_exp(optimizer, base_lr, ep, total_ep, start_decay_at_ep):
    ...
    for g in optimizer.param_groups:
        g['lr'] = (base_lr * (0.001 **
                              (float(ep + 1 - start_decay_at_ep) / (total_ep + 1 - start_decay_at_ep))))
...
def adjust_lr_warmup(optimizer, base_lrs, ep, decay_at_epochs, factor):
    assert ep >= 1, "Current epoch number should be >= 1"
    if ep < decay_at_epochs[0]:

        for i in range(len(optimizer.param_groups)):
            base_lr=base_lrs[i]
            optimizer.param_groups[i]['lr']=(base_lr) *(float(ep+1)/float(decay_at_epochs[0]))


        # for g in optimizer.param_groups:
        #     g['lr'] = (base_lr) *(float(ep+1)/float(decay_at_epochs[0]))

        for i in range(len(optimizer.param_groups)):
            g = optimizer.param_groups[i]
            print('=====> lr adjusted to {:.10f}'.format(g['lr']).rstrip('0'))
        return
    elif ep not in decay_at_epochs:
        return

    ind = find_index(decay_at_epochs, ep)
    for i in range(len(optimizer.param_groups)):
        base_lr = base_lrs[i]
        optimizer.param_groups[i]['lr'] = base_lr * factor ** (ind)
...
def adjust_lr_cosine(optimizer, base_lrs, ep, total_ep):
    assert ep >= 1, "Current epoch number should be >= 1"
    ep_after_warm = ep
    for i in range(len(optimizer.param_groups)):
        base_lr = base_lrs[i]
        new_lr = 0.5 *( 1.0 + math.cos( (ep_after_warm * math.pi) / total_ep ) ) * base_lr
        if new_lr < 0.0:
            new_lr = 0.0
...
# warm up + cosine lr strategy
def adjust_lr_warmup_cosine(optimizer, base_lrs, ep, warmup_ep, total_ep):
    assert ep >= 1, "Current epoch number should be >= 1"
    if ep < warmup_ep:

        for i in range(len(optimizer.param_groups)):
            base_lr=base_lrs[i]
            optimizer.param_groups[i]['lr']=(base_lr) *(float(ep+1)/float(warmup_ep))
    ...
    ep_after_warm = ep - warmup_ep
    total_ep = total_ep - warmup_ep
    for i in range(len(optimizer.param_groups)):
        base_lr = base_lrs[i]
        new_lr = 0.5 *( 1.0 + math.cos( (ep_after_warm * math.pi) / total_ep ) ) * base_lr
        if new_lr < 0.0:
...
def adjust_lr_warmup_SGDR(optimizer, base_lrs, ep, restart_eps, lr_max, lr_min, total_ep):
    assert ep >= 1, "Current epoch number should be >= 1"
    warmup_ep = restart_eps[0]
    if ep < warmup_ep:
    ...
    new_restart_eps = [item for item in restart_eps]
    new_restart_eps.append(total_ep)
    #check current restart stage
    cur_stage = 0
    for i in range(len(restart_eps)):
        if ep >= new_restart_eps[i] and ep < new_restart_eps[i+1]:
            cur_stage = i
            break
    cur_lr_max = lr_max[cur_stage]
    cur_lr_min = lr_min[cur_stage]
    cur_ep_run = ep - new_restart_eps[cur_stage]
    cur_stage_len = new_restart_eps[cur_stage+1] - new_restart_eps[cur_stage]

    for i in range(len(optimizer.param_groups)):
        new_lr = cur_lr_min + 0.5 *(cur_lr_max - cur_lr_min) * (1.0 + math.cos( (cur_ep_run * math.pi) / cur_stage_len) ) 
        if new_lr < 0.0:
            new_lr = 0.0
        optimizer.param_groups[i]['lr'] = new_lr
